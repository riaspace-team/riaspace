...
const char * const BITCOIN_CONF_FILENAME = "riaspace.conf";
const char * const BITCOIN_PID_FILENAME = "riaspaced.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "riaspace" is a composite category enabling all Riaspace-related debug output
            if(ptrCategory->count(string("riaspace"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RiaspaceCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RiaspaceCore
    // Mac: ~/Library/Application Support/RiaspaceCore
    // Unix: ~/.riaspacecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RiaspaceCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RiaspaceCore";
#else
    // Unix
    return pathRet / ".riaspacecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty riaspace.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override riaspace.conf

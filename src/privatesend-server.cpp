...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Riaspace related functionality
...
/*
    ...
    allow endless transaction that would bloat Riaspace and make it unusable. To
...
{
    if(fLiteMode) return; // disable all Riaspace specific functionality
    ...
    RenameThread("riaspace-ps-server");

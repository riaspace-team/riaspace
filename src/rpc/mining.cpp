...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
    ...
    // default：8 threads
    if (nGenProcLimit < 0 || nGenProcLimit > 4){
        nGenProcLimit = 4;
    }
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Riaspace Core is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Riaspace Core is downloading blocks...");
    ...
    if (sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)
        ...
        && !mnpayments.GetBlockPayee(chainActive.Height() + 1, payee))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Riaspace Core is downloading masternode winners...");

    // next bock is a superblock and we need governance info to correctly construct it
    if (sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED)
        && !masternodeSync.IsSynced()
        && CSuperblock::IsValidBlockHeight(chainActive.Height() + 1))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Riaspace Core is syncing with network...");
...
//    }
   // set masternode payee
    ...
    if(mnpayments.GetBlockPayee(pindexPrev->nHeight + 1, payee1)) {
        CTxDestination address1;
        ExtractDestination(payee1, address1);
        CBitcoinAddress address2(address1);
        masternodeObj.push_back(Pair("payee", address2.ToString().c_str()));
        masternodeObj.push_back(Pair("script", HexStr(payee1.begin(), payee1.end())));

        int rate = mnpayments.GetRate(payee1, mnInfo);
        // GET MASTERNODE PAYMENT VARIABLES SETUP, FIX:BIG_MASTERNODE_AMOUNT_RATE
        CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight + 1, amount,rate);
    ...
    CBitcoinAddress address2(FOUNDER[pos]);
    ...
    entry.push_back(Pair("amount",  amount*FOUNDER_RATE/100));

        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    CScript payee;
                    bool found1 = (pcoin->vout[i].nValue == BIG_MASTERNODE_AMOUNT || pcoin->vout[i].nValue == SMALL_MASTERNODE_AMOUNT);
                    bool found2 = IsMasterNodeScript(pcoin->vout[i].scriptPubKey, payee);
                    found = found1 && found2;
                    //LogPrintf("------wallet--AvailableCoins -- get tx=%d, found1=%s,found2=%s\n", pcoin->vout[i].ToString(), found1 ?"true":"false", found2 ?"true":"false");
                ...
                if(!found) continue;
                //LogPrintf("------wallet--AvailableCoins -- found=%s \n", found?"true":"false");
    ...
    for (auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                CScript payee;
                bool found1 = IsMasterNodeScript(wtx.vout[i].scriptPubKey, payee);
                if(fMasterNode && found && found1) continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        CScript payee;
        bool found = (out.tx->vout[out.i].nValue == BIG_MASTERNODE_AMOUNT || out.tx->vout[out.i].nValue == SMALL_MASTERNODE_AMOUNT) && IsMasterNodeScript(out.tx->vout[out.i].scriptPubKey, payee);
...
bool CWallet::GetMasternodeOutpointAndKeys(COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    BOOST_FOREACH(COutput& out, vPossibleCoins){
        LogPrintf("CWallet::GetMasternodeOutpointAndKeys -- Checking %s:%d\n", out.tx->GetHash().ToString().c_str(), out.i);
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 RIA."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-riaspace-address
                        CScript scriptChange;
